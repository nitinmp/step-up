...
import { loadWeekProgressCertificate } from "@/lib/certificate-service";
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    const certificate = await loadWeekProgressCertificate(
      session.user.id,
      weekNo,
    );
    ...
    return NextResponse.json(
      { error: "Could not generate progress report." },
      { status: 500 },
    );
  }
}
